...
INPUT_DIR="kraken"
if [ ! -d "$INPUT_DIR" ]; then
    echo "Error: Directory $INPUT_DIR not found. Please run this script from the project root."
    exit 1
fi
...
kraut single-report -i kraken/genome/Segatella_copri.tsv -o "$TEST_OUT_DIR/single_report.txt"
...
diff "$TEST_OUT_DIR/single_report.txt" kraken/genome/Segatella_copri.tsv
if [ $? -ne 0 ]; then
...
kraut merge-reports kraken/shredded-mixes-filtered/*.tsv -o "$TEST_OUT_DIR/merged.tsv"
...
kraut make-table kraken/shredded-mixes-filtered/*.tsv -o "$TEST_OUT_DIR/table_tot.tsv" --metric TOT --rank S --rank-prefix
...
if [ -f "kraken/shredded-mix-merge.tsv" ]; then
    kraut split-combine-table -i kraken/shredded-mix-merge.tsv -o "$TEST_OUT_DIR/split" --rank S --taxid
